package com.punedelights.user.helper
...
import com.punedelights.user.activity.PaymentActivity.Companion.hideProgressDialog
import com.punedelights.user.activity.PaymentActivity.Companion.showProgressDialog
import com.punedelights.user.adapter.OfferAdapter
import com.punedelights.user.model.OfferImages
...
class ApiConfig : Application() {
            ...
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    try {
                        ...
                        images.clear()
                        lytTopOfferImages.visibility =View.GONE
                    }
                }
                if (images.size > 0) {
                    lytTopOfferImages.visibility =View.VISIBLE
    ...
    fun placeOrder(
        ...
        if (isSuccess) {
            ...
            hideProgressDialog()
            addTransaction(activity,"", paymentType, transactionId, status, "Order Failed", sendParams1)
        }
    }
