...
import com.punedelights.user.adapter.DateAdapter
import com.punedelights.user.adapter.SlotAdapter
import com.punedelights.user.helper.*
import com.punedelights.user.helper.ApiConfig.Companion.getDates
...
import com.punedelights.user.model.BookingDate
import com.punedelights.user.model.Slot
...
class PaymentActivity : AppCompatActivity(), PaymentResultListener,
    ...
    private fun getAllWidget() {
        try {
            ...
            processLyt.weightSum = (if (from == "cart") 2.toFloat() else 3.toFloat())
    ...
    fun setPaymentMethod() {
        if (subtotal > 0) {
            if (Constant.DIRECT_BANK_TRANSFER == "0" && Constant.FLUTTER_WAVE == "0" && Constant.PAYPAL == "0" && Constant.COD == "0" && Constant.RAZORPAY == "0" && Constant.PAY_STACK == "0" && Constant.MIDTRANS == "0" && Constant.STRIPE == "0" && Constant.PAYTM == "0") {
                lytPayOption.visibility = View.GONE
            } else {
                lytPayOption.visibility = View.VISIBLE
                if (Constant.COD == "1") {
            ...
            override fun onSuccess(result: Boolean, response: String) {
                if (result) {
                    try {
                        ...
                        paramMap[Constant.ORDER_ID_] = jsonObject.getString(Constant.ORDER_ID)
