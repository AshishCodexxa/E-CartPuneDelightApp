...
import com.punedelights.user.fragment.CartFragment
    ...
    private fun removeItem(position: Int) {
        if (isLogin) {
            ...
            if (Constant.CartValues.containsKey(cart.product_variant_id)) {
                ...
                Constant.CartValues[cart.product_variant_id] = "0"
            }
            carts.remove(cart)
            createHashMap()
    ...
    override fun onBindViewHolder(holderParent: RecyclerView.ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            viewTypeItem -> {
                ...
                Glide.with(activity).load(if (cart.image == "") "-" else cart.image)
            ...
            if (session.getData(Constant.STATUS) == "1") {
                ...
                if (count <= maxCartCont.toInt()) {
                    if (availableStock >= unit) {
                        ...
                        Cart(cart.product_id, cart.product_variant_id, cart.qty)
                        for (cart1 in Constant.countList) {
